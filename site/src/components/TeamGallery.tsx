...
const teamMembers = [
  ...
  },
  {
    ...
    color: "#fef9c3",   // clair
    color2: "#facc15",  // intermédiaire
    color3: "#eab308",  // intense
    text: "Support & Tests",
    href: "/steven",
...
export default function TeamGallery() {
  return (
    <ColoredSection id="team" className="py-24 px-6 text-center relative" colorClass="bg-green-500">
      ...
      <h2 className="text-3xl md:text-4xl font-bold mb-10 text-white relative z-10">
        Notre Équipe
      </h2>
      <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 max-w-7xl mx-auto relative z-10">
        {teamMembers.map((member, index) => (
          <a
            key={index}
            href={member.href}
            className="relative overflow-hidden rounded-xl h-40 flex items-center justify-center bg-neutral-900 text-white group transition transform hover:scale-105 hover:shadow-xl"
          >
            {/* Couches d'animation */}
            <span
